...
export const initWsTokenQueryKey = (options: Options<InitWsTokenData>) => createQueryKey('initWsToken', options);
 ...
 */
export const initWsTokenOptions = (options: Options<InitWsTokenData>) => {
    return queryOptions({
        queryFn: async ({ queryKey, signal }) => {
            const { data } = await initWsToken({
                ...options,
                ...queryKey[0],
                signal,
                throwOnError: true
        ...
        queryKey: initWsTokenQueryKey(options)
    });
};
