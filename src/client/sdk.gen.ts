 ...
 * Usage:
 *
 * 1) Connect: perform WS upgrade to this endpoint with valid auth.
 * 2) Server hello: {"op":"hello","conn_id":"...","heartbeat_ms":30000,"max_backlog":500}.
 * 3) Subscribe to ticket updates: {"op":"subscribe","topics":["ticket:{ticket_uuid}"]}.
 * 4) Receive events: {"op":"event","topic":"ticket:{ticket_uuid}","seq":1042,"type":"message.created","ts":"2025-08-01T13:20:00Z","data":{...}}.
 * 5) Resume missed events (by org-scoped sequence id): {"op":"resume","from":1040}. Server will emit buffered events with seq > from for currently subscribed topics before live events.
 * 6) Heartbeat: server pings every 30s; client may send {"op":"ping"} and will receive {"op":"pong"}.
 *
 * Topics:
 * - ticket:{uuid} — subscribe to a single ticket's changes (messages, watchers, tags, locks, team assignment, drafts, status/priority/assignee, splits/merges, etc.).
 *
 * Client envelopes (JSON):
 * - subscribe: {"op":"subscribe","topics":["ticket:{uuid}"]}
 * - unsubscribe: {"op":"unsubscribe","topics":["ticket:{uuid}"]}
 * - resume: {"op":"resume","from":1042}
 * - ping: {"op":"ping","ts":"2025-08-01T12:34:56Z"}
 *
 * Server envelopes (JSON):
 * - hello: {"op":"hello","conn_id":"...","heartbeat_ms":30000,"max_backlog":500}
 * - event: {"op":"event","topic":"ticket:{uuid}","seq":1042,"type":"ticket.locked","ts":"...","data":{...}}
 * - pong: {"op":"pong","ts":"..."}
 * - error: {"op":"error","code":"INVALID_TOPIC","message":"Only ticket:{uuid} supported"}
 */
 ...
 */
export const initWsToken = <ThrowOnError extends boolean = false>(options: Options<InitWsTokenData, ThrowOnError>) => {
    return (options.client ?? _heyApiClient).get<InitWsTokenResponses, unknown, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
