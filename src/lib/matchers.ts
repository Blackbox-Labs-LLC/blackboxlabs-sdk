...
import { BlackboxDefaultError } from "../models/errors/blackboxdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | BlackboxDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | BlackboxDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new BlackboxDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new BlackboxDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
